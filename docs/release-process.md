...
`release-dry-run` depends on `make validate`. It does **not** run network requests, query GitHub branch protection/status APIs, create a tag, push Git state, create a GitHub Release, or upload an asset. The required remote Solo VPS `fast-source` status therefore remains an explicit GitHub release prerequisite rather than something an offline dry run can fabricate. The upstream repository is now `https://github.com/paracosm17/solo-vps`; its hosted check and branch policy still require separate verification. A green consumer-application workflow cannot substitute for it.

...
**PRE-ALPHA:** пока используйте тестовый VPS. Проверенного публичного релиза ещё нет. Для установки нужна выбранная копия проекта.
...
| `REPOSITORY_URL` | URL репозитория Solo VPS |
...
Выберите формат исходников, который получили. После появления публичного репозитория и тега `v0.1.0` используйте путь Git с настоящим URL репозитория и полным commit ID, на который указывает тег.

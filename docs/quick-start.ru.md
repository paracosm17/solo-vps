...
Для текущего тестирования склонируйте публичную ветку `main`. Когда появится релизный тег, используйте `git clone --branch v0.1.0 --depth 1 https://github.com/paracosm17/solo-vps.git solo-vps`. ZIP подходит только для отдельно предоставленного тестового архива.
...
=== "Git"
    ...
    **На VPS под root:**
    ...
    Теперь вы в каталоге Solo VPS. Для записи точной версии в отчёте выполните `git rev-parse HEAD`; вводить её при установке не нужно.

#!/usr/bin/env python3
"""Create, validate, and deliver the SOPS-encrypted Grafana Cloud Metrics credential bundle."""
...
try:
    from scripts.metrics_credentials_common import MetricsCredentialError, canonical_json_bytes, parse_json_bytes
    ...
    from metrics_credentials_common import MetricsCredentialError, canonical_json_bytes, parse_json_bytes
...
def _validate_external_ciphertext_path(root: Path, output: Path) -> Path:
    root = root.resolve()
    output = _absolute_unresolved(output)
    if output.is_symlink():
        raise BundleError(f"refusing symlink metrics ciphertext destination: {output}")
    if _inside(output, root):
        raise BundleError("metrics ciphertext for an installation must stay outside the public source checkout")
    return output


def _write_external_ciphertext(root: Path, output: Path, encrypted: bytes) -> None:
    output = _validate_external_ciphertext_path(root, output)
    if output.exists():
        raise BundleError(f"refusing to overwrite existing metrics ciphertext: {output}")
...
def _new_payload() -> dict[str, str]:
    print("Enter the Grafana Cloud Metrics connection values. Secret values are not echoed back.")
    prometheus_url = input("Grafana Cloud Prometheus remote write URL: ").strip()
    prometheus_username = input("Grafana Cloud Metrics user ID: ").strip()
    api_key = getpass.getpass("Grafana Cloud access-policy token (metrics:write): ")
    payload = {
        "PROMETHEUS_URL": prometheus_url,
        "PROMETHEUS_USERNAME": prometheus_username,
        "GRAFANA_CLOUD_METRICS_API_KEY": api_key,
    }
    try:
        return json.loads(canonical_json_bytes(payload).decode("utf-8"))
    except MetricsCredentialError as exc:
        raise BundleError(str(exc)) from exc


def init_bundle(root: Path, policy: Path, output: Path, key_file: Path, sops: Path) -> None:
    if not policy.is_file():
        raise BundleError(f"persistent SOPS policy is missing: {policy}; initialize it first")
    output = _validate_external_ciphertext_path(root, output)
    if output.exists():
        payload = decrypt_bundle(policy, output, key_file, sops)
        print("PASS encrypted metrics secret bundle already exists; reusing it")
    ...
    print("PASS encrypted metrics secret bundle created")
    print(f"  ciphertext: {output.expanduser().resolve()}")
...
def decrypt_bundle(policy: Path, encrypted: Path, key_file: Path, sops: Path) -> dict[str, str]:
    ...
    if not encrypted.is_file() or encrypted.is_symlink():
        raise BundleError(f"encrypted metrics secret bundle is missing or unsafe: {encrypted}")
    ...
    try:
        ...
        raise BundleError(f"decrypted metrics bundle failed schema validation: {exc}") from exc
...
def push_bundle(
    ...
    *,
    ...
    remote = f"cd {remote_dir} && sudo -n python3 scripts/install_metrics_credentials.py apply"
    ...
    if stdout:
        print(stdout)
    print("PASS workstation-to-VPS metrics credential delivery")
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "init":
            init_bundle(args.root, policy, encrypted, key_file, sops)
        else:
            payload = decrypt_bundle(policy, encrypted, key_file, sops)
            if args.command == "check":
                print("PASS encrypted metrics secret bundle decrypts and matches the expected schema")
    ...
    except (BundleError, MetricsCredentialError, ToolchainError, OSError) as exc:
        print(f"ERROR metrics secret bundle: {exc}", file=sys.stderr)
        return 2
    return 0

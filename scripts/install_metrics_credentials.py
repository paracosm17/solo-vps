#!/usr/bin/env python3
"""Install or verify root-only Grafana Cloud Metrics credentials on the VPS."""
...
try:
    ...
    from metrics_credentials_common import (
        MAX_JSON_BYTES,
        MetricsCredentialError,
        canonical_json_bytes,
        parse_json_bytes,
...
def _lstat_regular(path: Path) -> os.stat_result | None:
    ...
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISREG(metadata.st_mode):
        raise InstallError(f"refusing non-regular metrics credential path: {path}")
    return metadata
...
def _prepare_directory(directory: Path, *, owner_uid: int = 0, owner_gid: int = 0) -> None:
    ...
    if stat.S_ISLNK(metadata.st_mode) or not stat.S_ISDIR(metadata.st_mode):
        raise InstallError(f"refusing non-directory metrics credential root: {directory}")
    os.chown(directory, owner_uid, owner_gid)
    os.chmod(directory, 0o700)


def install_payload(payload: dict[str, str], destination: Path = DEFAULT_PATH, *, owner_uid: int = 0, owner_gid: int = 0) -> bool:
    ...
    fd, temporary_name = tempfile.mkstemp(prefix=".grafana-cloud-metrics.", dir=directory)
...
def verify_file(destination: Path = DEFAULT_PATH, *, owner_uid: int = 0, owner_gid: int = 0) -> dict[str, str]:
    directory = destination.parent
    try:
        directory_meta = directory.lstat()
    except FileNotFoundError as exc:
        raise InstallError(f"metrics credential directory is missing: {directory}") from exc
    if stat.S_ISLNK(directory_meta.st_mode) or not stat.S_ISDIR(directory_meta.st_mode):
        raise InstallError(f"metrics credential root is not a real directory: {directory}")
    if directory_meta.st_uid != owner_uid or directory_meta.st_gid != owner_gid:
        raise InstallError("metrics credential directory has unexpected ownership")
    if stat.S_IMODE(directory_meta.st_mode) != 0o700:
        raise InstallError("metrics credential directory mode must be 0700")

    metadata = _lstat_regular(destination)
    if metadata is None:
        raise InstallError(f"metrics credential file is missing: {destination}")
    if metadata.st_uid != owner_uid or metadata.st_gid != owner_gid:
        raise InstallError("metrics credential file has unexpected ownership")
    if stat.S_IMODE(metadata.st_mode) != 0o600:
        raise InstallError("metrics credential file mode must be 0600")
    try:
        return parse_json_bytes(destination.read_bytes())
    except MetricsCredentialError as exc:
        raise InstallError(f"metrics credential file schema is invalid: {exc}") from exc


def _read_stdin() -> dict[str, str]:
    raw = sys.stdin.buffer.read(MAX_JSON_BYTES + 1)
    if len(raw) > MAX_JSON_BYTES:
        raise InstallError("metrics credential stdin payload exceeds 32 KiB")
    try:
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "apply":
            ...
            print("PASS metrics runtime credentials installed")
            print(f"  path: {destination}")
            print("  owner: root:root")
            print("  mode: 0600")
            print(f"  changed: {'yes' if changed else 'no'}")
            print("  plaintext printed: no")
            print("  metrics service started: no")
            print("  telemetry sent: no")
        else:
            payload = verify_file(destination)
            print("PASS metrics runtime credentials")
            print(f"  path: {destination}")
            print("  owner: root:root")
            print("  mode: 0600")
        ...
        print(f"ERROR metrics runtime credentials: {exc}", file=sys.stderr)
        return 2
    return 0

...
$remoteCommand = "cd $RemoteProject && sudo -n python3 scripts/install_observability_credentials.py apply"
...
if ($process.ExitCode -ne 0) {
    ...
    if ($detail) { throw "VPS observability credential installation failed: $detail" }
    throw "VPS observability credential installation failed with exit code $($process.ExitCode)."
}
if ($stdout) { Write-Host $stdout.TrimEnd() }
Write-Host "PASS workstation-to-VPS observability credential delivery"

...
Assert-BackupPayload -Payload $payload
$payload = $null

$remoteCommand = "cd $RemoteProject && sudo -n python3 scripts/install_backup_credentials.py apply"
...
if ($process.ExitCode -ne 0) {
    ...
    if ($detail) { throw "VPS credential installation failed: $detail" }
    throw "VPS credential installation failed with exit code $($process.ExitCode)."
}
if ($stdout) { Write-Host $stdout.TrimEnd() }
Write-Host "PASS workstation-to-VPS backup credential delivery"

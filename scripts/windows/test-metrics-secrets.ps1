...
function Assert-MetricsPayload {
    ...
    $uri = $null
    if (-not [Uri]::TryCreate([string]$Payload.PROMETHEUS_URL, [UriKind]::Absolute, [ref]$uri) -or $uri.Scheme -ne "https" -or -not $uri.AbsolutePath.TrimEnd("/").EndsWith("/api/prom/push")) {
        throw "Decrypted PROMETHEUS_URL does not match the expected HTTPS Prometheus remote-write endpoint."
    }
...
$keyCount = Assert-MetricsPayload -Payload $payload
$json = $null
$payload = $null

Write-Host "PASS encrypted metrics secret bundle decrypts and matches the expected schema"

#!/usr/bin/env python3
"""Create, validate, and deliver the M24 SOPS-encrypted observability credential bundle."""
...
try:
    from scripts.observability_credentials_common import ObservabilityCredentialError, canonical_json_bytes, parse_json_bytes
    ...
    from observability_credentials_common import ObservabilityCredentialError, canonical_json_bytes, parse_json_bytes
...
def _validate_external_ciphertext_path(root: Path, output: Path) -> Path:
    ...
    if output.is_symlink():
        raise BundleError(f"refusing symlink observability ciphertext destination: {output}")
    if _inside(output, root):
        raise BundleError("observability ciphertext for an installation must stay outside the public source checkout")
    return output
...
def _write_external_ciphertext(root: Path, output: Path, encrypted: bytes) -> None:
    ...
    if output.exists():
        raise BundleError(f"refusing to overwrite existing observability ciphertext: {output}")
...
def _new_payload() -> dict[str, str]:
    print("Enter the Grafana Cloud Logs connection values. Secret values are not echoed back.")
    loki_url = input("Grafana Cloud Loki push URL: ").strip()
    loki_username = input("Grafana Cloud Logs user ID: ").strip()
    api_key = getpass.getpass("Grafana Cloud access-policy token (logs:write): ")
    payload = {
        "LOKI_URL": loki_url,
...
def init_bundle(root: Path, policy: Path, output: Path, key_file: Path, sops: Path) -> None:
    ...
    if output.exists():
        ...
        print("PASS encrypted observability secret bundle already exists; reusing it")
    ...
    print("PASS encrypted observability secret bundle created")
    print(f"  ciphertext: {output.expanduser().resolve()}")
...
def decrypt_bundle(policy: Path, encrypted: Path, key_file: Path, sops: Path) -> dict[str, str]:
    ...
    if not encrypted.is_file() or encrypted.is_symlink():
        raise BundleError(f"encrypted observability secret bundle is missing or unsafe: {encrypted}")
    ...
    try:
        ...
        raise BundleError(f"decrypted observability bundle failed schema validation: {exc}") from exc
...
def push_bundle(
    ...
    *,
    ...
    remote = f"cd {remote_dir} && sudo -n python3 scripts/install_observability_credentials.py apply"
    ...
    if stdout:
        print(stdout)
    print("PASS workstation-to-VPS observability credential delivery")
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "init":
            ...
            if args.command == "check":
                print("PASS encrypted observability secret bundle decrypts and matches the expected schema")
    ...
    except (BundleError, ObservabilityCredentialError, ToolchainError, OSError) as exc:
        print(f"ERROR observability secret bundle: {exc}", file=sys.stderr)
        return 2
    return 0

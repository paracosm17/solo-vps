...
Before the first push, scan the exact refs/history intended for publication with the built-in history check and an independent maintained secret scanner; review every finding. Then push the reviewed repository to its intended GitHub home, set `main` as the default branch, enable `Repository CI / fast-source` as a required check, set Pages source to GitHub Actions and verify the deployed URLs, and enable/test private vulnerability reporting. A demo application's CI result does not satisfy the repository CI gate.
